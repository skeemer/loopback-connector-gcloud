...
GcloudDataStoreConnector.prototype.find = function(model, filter, callback) {
    ...
    if (filter.where && filter.where.hasOwnProperty('or')) {
        ...
        if (undefined !== filter.order) {
            debug('find: adding order %s', filter.order);
            debug('find: adding order %s %s', filter.order.split(" ")[0], filter.order.split(" ")[1]);
            var field = filter.order.split(" ")[0];
            var orderBy = false;
            if(filter.order.split(" ")[1] == 'DESC') {
              orderBy = true;
            }

            query = query.order(field, '{ descending: ' + orderBy + ' }');
        }
